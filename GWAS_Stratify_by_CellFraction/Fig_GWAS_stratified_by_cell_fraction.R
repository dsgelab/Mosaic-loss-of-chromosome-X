### For 49 independent loci identified by mLOX GWAS meta-analysis, we investigated whether the effects were shared by different cell fractions or age onset.
## (1) cell fraction below 5% and above 5%;
## (2) 5 groups for cell fractions;
## (3) 5 groups for age onset (genotyping age)


##################################################################################################
#                             Run statified GWAS in FinnGen                                      #
##################################################################################################

# the same control group was used for GWAS statified by cell fraction; but not for GWAS stratified by genotyping age.
...
###############################################################################################################
#         Bar plot with P-value from pair-wise comparisons of CF-stratified analysis (5 groups, wrong)        #
###############################################################################################################

# gsutil cp  gs://dsge-aoxing/mocha/LOX_GWAS/Meta_mLOX/EachBiobank/mLOX_FinnGenr9_CellFractionStrata.49loci.20220527.tsv  /Users/aoxliu/Documents/Project2_Finngen_mCA/Analysis_GWAS_LOX/Manuscript/Plots/input


## Read in and format cf-stratified results -----------
stats_cf <- read.table("../input/mLOX_FinnGenr9_CellFractionStrata.49loci.20220527.tsv", header=T)
stats_cf <- stats_cf %>% mutate(Variant=paste0(chrom,":",pos,":",ref,":",alt)) %>% rename(cf="CellFraction")
dim(stats_cf)    # 215  18

n_variant <- nrow(stats_cf)/5
stats_compare <- matrix(NA, nrow=n_variant*10, ncol=11)
colnames(stats_compare) <- c("Variant", "cf_1", "cf_2", "beta_cf_1", "beta_cf_2", "se_cf_1", "se_cf_2",  "p_cf_1", "p_cf_2", "p_diff", "sig_diff")


stats_compare[,"Variant"] <- rep(unique(stats_cf$Variant), each=10)
cf_grp1 <- c("000_015", "000_015", "000_015", "000_015", 
             "015_020", "015_020", "015_020", 
             "020_025", "020_025", 
             "025_035")

cf_grp2 <- c("015_020", "020_025", "025_035", "035_999",
             "020_025", "025_035", "035_999",
             "025_035", "035_999",
             "035_999")

stats_compare[,"Variant"] <- rep(unique(stats_cf$Variant), each=10)
stats_compare[,"cf_1"] <- cf_grp1
stats_compare[,"cf_2"] <- cf_grp2
stats_compare <- data.frame(stats_compare)

for (i in 1:nrow(stats_compare)) {
	print(i)
	variant <- stats_compare[i,"Variant"]
	cf1 <- stats_compare[i,"cf_1"]
	cf2 <- stats_compare[i,"cf_2"]
	
	d_cf1 <- stats_cf %>% filter(Variant==variant & cf==cf1)
	d_cf2 <- stats_cf %>% filter(Variant==variant & cf==cf2)
	
	stats_compare[i, "beta_cf_1"] <- d_cf1[1,"beta"]
	stats_compare[i, "beta_cf_2"] <- d_cf2[1,"beta"]
	
	stats_compare[i, "se_cf_1"] <- d_cf1[1,"sebeta"]
	stats_compare[i, "se_cf_2"] <- d_cf2[1,"sebeta"]
	
	stats_compare[i, "p_cf_1"] <- d_cf1[1,"pval"]
	stats_compare[i, "p_cf_2"] <- d_cf2[1,"pval"]
	
	as.numeric(as.character(stats_compare[i, "beta_cf_1"]))
	beta_diff <- as.numeric(as.character(stats_compare[i, "beta_cf_1"])) -  as.numeric(as.character(stats_compare[i, "beta_cf_2"]))
	var_diff <- as.numeric(as.character(stats_compare[i, "se_cf_1"]))^2 + as.numeric(as.character(stats_compare[i, "se_cf_2"]))^2
...
stats_compare <- stats_compare %>% inner_join(sig_LOX_lst[,c("Variant", "rsid", "Top_gene", "CHR", "BP")], by="Variant") %>%
      mutate(beta_cf_1=round(as.numeric(as.character(beta_cf_1)), 3), 
             beta_cf_2=round(as.numeric(as.character(beta_cf_2)), 3), 
             se_cf_1=round(as.numeric(as.character(se_cf_1)), 3),
             se_cf_2=round(as.numeric(as.character(se_cf_2)), 3),
             
             cf_1=sub("000_015","<1.5%",cf_1), 
             cf_1=sub("015_020","1.5%-2%",cf_1), 
             cf_1=sub("020_025","2%-2.5%",cf_1), 
             cf_1=sub("025_035","2.5%-3.5%",cf_1), 
             cf_1=sub("035_999",">3.5%",cf_1), 
             
             cf_2=sub("000_015","<1.5%",cf_2), 
             cf_2=sub("015_020","1.5%-2%",cf_2), 
             cf_2=sub("020_025","2%-2.5%",cf_2), 
             cf_2=sub("025_035","2.5%-3.5%",cf_2), 
             cf_2=sub("035_999",">3.5%",cf_2), 
            
             p_cf_1=as.numeric(as.character(p_cf_1)),
             p_cf_2=as.numeric(as.character(p_cf_2)),
             p_diff=as.numeric(as.character(p_diff)),
             p_cf_1=ifelse(round(p_cf_1,2)<0.01, formatC(p_cf_1, format="e", digits=2), round(p_cf_1,2)),
             p_cf_2=ifelse(round(p_cf_2,2)<0.01, formatC(p_cf_2, format="e", digits=2), round(p_cf_2,2)),
             p_diff=ifelse(round(p_diff,2)<0.01, formatC(p_diff, format="e", digits=2), round(p_diff,2))) %>% 
      arrange(desc(-CHR), desc(-BP)) %>% 
      select(Variant, Top_gene, 
             cf_1, beta_cf_1, se_cf_1, p_cf_1, 
             cf_2, beta_cf_2, se_cf_2, p_cf_2, p_diff)
write.table(stats_compare, "../input/G2G_LOX_GeneScores_49Signals_Signals.CellFraction.hg38.tsv", append=F, quote=F, sep="\t", row.names=F)


### function to plot pair-wise comparions -------------------
plot_cf <- function(SNP){
	GENE <- unlist(sig_LOX_lst %>% filter(Variant==SNP) %>% select(Top_gene))
	
	## check the beta from meta-analysis, if is negative, change it to the effect of risk allele --------------
	stats_cf_risk <- stats_cf %>% filter(Variant==SNP) %>% 
	            # mutate(beta_risk=beta*risk_dir, 
	            mutate(# beta_risk=-beta, 
	                   sig=ifelse(pval<0.05, "T", "F")) %>% 
	            mutate(cf=sub("000_015","<1.5%",cf), 
	                   cf=sub("015_020","1.5%-2%",cf), 
	                   cf=sub("020_025","2%-2.5%",cf), 
	                   cf=sub("025_035","2.5%-3.5%",cf), 
	                   cf=sub("035_999",">3.5%",cf), 
	                   cf=factor(cf, levels=c("<1.5%", "1.5%-2%", "2%-2.5%", "2.5%-3.5%", ">3.5%")) ) %>% 
	            arrange(-desc(cf))
	
	BETA <- unlist(sig_LOX_lst %>% filter(Variant==SNP) %>% select(BETA))
	if (BETA>0) {
		stats_cf_risk <- stats_cf_risk %>% mutate(beta_risk=-beta)
	} else {
		stats_cf_risk <- stats_cf_risk %>% mutate(beta_risk=beta)
	}
	
	pair_test <- stats_compare %>% filter(Variant==SNP) %>% 
	        mutate(group1=sub("000_015","<1.5%",cf_1), 
	               group1=sub("015_020","1.5%-2%",group1), 
	               group1=sub("020_025","2%-2.5%",group1), 
	               group1=sub("025_035","2.5%-3.5%",group1), 
	               group1=sub("035_999",">3.5%",group1), 
	               
	               group2=sub("000_015","<1.5%",cf_2), 
	               group2=sub("015_020","1.5%-2%",group2), 
	               group2=sub("020_025","2%-2.5%",group2), 
	               group2=sub("025_035","2.5%-3.5%",group2), 
	               group2=sub("035_999",">3.5%",group2) ) %>% 
	...
	beta_max <- max(stats_cf_risk$beta_risk + stats_cf_risk$sebeta)
	if (nrow(pair_test)==1) {pair_test[,"y.position"] <- beta_max+0.05 }
	if (nrow(pair_test)==2) {pair_test[,"y.position"] <- c(beta_max+0.05, beta_max+0.1) }
	if (nrow(pair_test)==3) {pair_test[,"y.position"] <- c(beta_max+0.05, beta_max+0.1, beta_max+0.15) }
	if (nrow(pair_test)==4) {pair_test[,"y.position"] <- c(beta_max+0.05, beta_max+0.1, beta_max+0.15, beta_max+0.2) }
	...
	p_onset_effect_test <- ggplot(stats_cf_risk) + 
		geom_bar(aes(x=cf, y=beta_risk, fill=sig), size=0.5, width=0.5, stat="identity", alpha=0.6) + 
		geom_errorbar(aes(x=cf, ymin=beta_risk-sebeta, ymax=beta_risk+sebeta, color=sig), width=0.1, size=0.5, linetype=1) + 
		geom_hline(yintercept=0, col="grey", size=0.5) + 
		labs(y="log(OR) of LOX", x="Cell fraction", title=tit) +
		scale_color_manual(breaks=c("T", "F"), values=c("#E41A1C", "black")) +
		scale_fill_manual(breaks=c("T", "F"), values=c("#E41A1C", "black")) +
		scale_size_manual(breaks=c("T", "F"), values=c(2,1)) +
		theme_classic() +
	...
	return(p_onset_effect_test)
	# ggsave(paste0("figures/SuppFigure2_",SNP,".",GENE,".CellFraction.tiff"), p_onset_effect_test, width=5, height=5)
}


pdf("figures/SuppFigure2_CellFractionStratifiedEffect.pdf")
for (variant in unique(stats_compare$Variant)){
	print(variant)
	print(plot_cf(variant))
}
dev.off()
...
############################################################################################################################
#         Scatter plot for CF-stratified analysis (2 grps, above 5% and below 5%)       #
############################################################################################################################

# gsutil cp  gs://dsge-aoxing/mocha/LOX_GWAS/Meta_mLOX/EachBiobank/mLOX_FinnGenr9_CellFractionStrata2grps.49loci.20220527.tsv  /Users/aoxliu/Documents/Project2_Finngen_mCA/Analysis_GWAS_LOX/Manuscript/Plots/input
setwd("/Users/aoxliu/Documents/Project2_Finngen_mCA/Analysis_GWAS_LOX/Manuscript/Plots/figures")
library(ggplot2)
library(dplyr)
library(tidyr)
...
## Read in and format cf-stratified results -----------
stats_cf <- read.table("../input/mLOX_FinnGenr9_CellFractionStrata2grps.49loci.20220527.tsv", header=T)
stats_cf <- stats_cf %>% mutate(Variant=paste0(chrom,":",pos,":",ref,":",alt)) %>% rename(cf="CellFraction")
dim(stats_cf)    # 86 19

n_variant <- nrow(stats_cf)/2
stats_compare <- matrix(NA, nrow=n_variant*1, ncol=11)
colnames(stats_compare) <- c("Variant", "cf_1", "cf_2", "beta_cf_1", "beta_cf_2", "se_cf_1", "se_cf_2",  "p_cf_1", "p_cf_2", "p_diff", "sig_diff")


stats_compare[,"Variant"] <- rep(unique(stats_cf$Variant), each=1)
cf_grp1 <- c("below5")
cf_grp2 <- c("above5")
...
stats_compare <- stats_compare %>% inner_join(sig_LOX_lst[,c("Variant", "rsid", "Top_gene", "CHR", "BP")], by="Variant") %>%
      mutate(beta_cf_1=round(as.numeric(as.character(beta_cf_1)), 3), 
             ...
             cf_1=sub("below5","<5%",cf_1), 
             cf_2=sub("above5",">5%",cf_2), 
      ...
      select(Variant, Top_gene, 
             cf_1, beta_cf_1, se_cf_1, p_cf_1, 
             cf_2, beta_cf_2, se_cf_2, p_cf_2, p_diff)
write.table(stats_compare, "../input/G2G_LOX_GeneScores_49Signals_Signals.CellFractionStrata2grps.hg38.tsv", append=F, quote=F, sep="\t", row.names=F)

cor(stats_compare$beta_cf_1, stats_compare$beta_cf_2)   # 0.8716635
plot(stats_compare$beta_cf_1, stats_compare$beta_cf_2)


# test for same effects
r <- 0.8716635
stats_compare <- stats_compare %>% mutate(v=se_cf_1^2 + se_cf_2^2 - 2*se_cf_1*se_cf_2*r,
                                          P_same=pchisq( (beta_cf_1 - beta_cf_2)^2/v, df=1, lower=F), 
                                          sig=ifelse(P_same<0.05/nrow(stats_compare), "P<0.00125", 
                                              ifelse(P_same<0.05, "P<0.05", "No difference")))

stats_compare %>% filter(sig %in% c("P<0.00125","P<0.05")) %>% arrange(desc(-P_same)) %>% select(-v, -sig)

p <- ggplot(stats_compare, aes(x=beta_cf_1, y=beta_cf_2, label=Top_gene)) + 
     geom_point(aes(color=sig, alpha=sig)) + 
     geom_errorbar(aes(xmin=beta_cf_1-1.96*se_cf_1, xmax=beta_cf_1+1.96*se_cf_1, color=sig, alpha=sig), size=0.35, linetype=1) + 
     geom_errorbar(aes(ymin=beta_cf_2-1.96*se_cf_2, ymax=beta_cf_2+1.96*se_cf_2, color=sig, alpha=sig), size=0.35, linetype=1) + 
     scale_color_manual(breaks=c("P<0.00125","P<0.05","No difference"), values=c("indianred1","dodgerblue","gray90")) + 
     scale_alpha_manual(breaks=c("P<0.00125","P<0.05","No difference"), values=c(1, 1, 0.4)) + 
     geom_abline(intercept=0, slope=1, linetype="dotted", size=0.5, colour="grey") + 
     geom_hline(yintercept=0, linetype="dotted", color="grey", size=0.35) +
     geom_vline(xintercept=0, linetype="dotted", color="grey", size=0.35) +
     labs(title="Lead variants stratified by cell fraction", x="log(OR) for mLOX with cell fraction < 5%", y="log(OR) for mLOX with cell fraction > 5%") +
     ...
     theme(legend.position=c(0.8,0.15), legend.title=element_text(size=8, face="bold"), legend.text=element_text(size=7), legend.key.size = unit(0.4, "cm")) + 
     guides(alpha=F, color=guide_legend(title="Stratified effect", cex=0.8)) + 
     geom_text_repel(data=stats_compare %>% filter(sig=="P<0.05"), color="dodgerblue", size=2.5, box.padding=0.6, point.padding=NA, segment.size=0.26, force=1, max.overlaps=Inf, min.segment.length=0, fontface="bold") + 
     geom_text_repel(data=stats_compare %>% filter(sig=="P<0.00125"), color="indianred1", size=2.5, box.padding=1.5, segment.size=0.26, fontface="bold") 
p  
ggsave("Figure2B_LOX_Leadvariants_CellFractionStratified_FinnGen_newcf.tiff", p, width=4.5, height=4.5)
